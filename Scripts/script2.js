...
function main() {
    let output_2ex = document.getElementById('output_2ex');
    let answerInput = document.getElementById('answerInput_2ex');
    ...
    let encode = [];
    outputans_2ex.innerHTML = ``;
    answer_2ex.innerHTML = '';
    ...
    output_2ex.innerHTML = encoding_type['description'] + "<br>";
    if (encoding_type["description"] === "Ваня шифрует русские слова, записывая вместо каждой буквы ее номер в алфавите (без пробелов)." ||
        ...
        output_2ex.innerHTML += "<br>Кодовая таблица:<br>";
        let count = 0;
        for (let [letter, code] of Object.entries(full_table)) {
            output_2ex.innerHTML += `${letter} ${code}&nbsp;&nbsp;`;
            if (++count % 4 === 0) {
                output_2ex.innerHTML += "<br>";
            }
        }

        output_2ex.innerHTML += "<br>Некоторые шифровки можно расшифровать не одним способом. Даны четыре шифровки:<br>";
        ...
        for (let message of encoded_messages) {
            output_2ex.innerHTML += message + "<br>";
        }

        if (index === 7) {
            output_2ex.innerHTML += "<br>Только одна из них расшифровывается единственным способом. Найдите ее и расшифруйте. В качестве ответа запишите количество БУКВ в сообщении.";
            answ = 'Введите количество букв: ';
        } else {
            output_2ex.innerHTML += "<br>Только одна из них расшифровывается единственным способом. Найдите ее и расшифруйте. Получившееся слово запишите в качестве ответа.";
            answ = 'Введите расшифрованное сообщение: ';
        }
        
    } else {
        output_2ex.innerHTML += "Кодовая таблица:<br>";
        for (let [letter, code] of shuffled_table) {
            output_2ex.innerHTML += `${letter}: ${code}<br>`;
        }
        output_2ex.innerHTML += `Закодированное сообщение: ${encoded_message}<br>`;
        if (index === 7) {
            output_2ex.innerHTML += "Сколько БУКВ в сообщении, если присутствуют только буквы из таблицы, которые не повторяются?<br>";
            answ = 'Введите количество букв: ';
        } else {
            output_2ex.innerHTML += "Какое сообщение закодировано, если присутствуют только буквы из таблицы, которые не повторяются?<br>";
            answ = 'Введите расшифрованное сообщение: ';
        }
    }
...
document.getElementById('generate_variant').addEventListener('click', main);


document.getElementById('save_checking').addEventListener('click', function() {
    console.log("Кнопка отправки нажата"); // Проверка привязки обработчика

    let outputans = document.getElementById('outputans_2ex');
    let answerInput = document.getElementById('answerInput_2ex');
    ...
    if ((expectedInput === "length" && parseInt(answer) === correctAnswer.length) || 
        ...
        fetch('/user/progress/informatics/2', {
            method: 'POST',
            headers: {
	...
	let answer = document.getElementById('answer_2ex');
    	let answerInput = document.getElementById('answerInput_2ex');
    	let correctAnswer = answerInput.getAttribute("data-correct-answer").toUpperCase();
    	let expectedInput = answerInput.getAttribute("data-expected-input");
...
document.getElementById('show_answer_2ex').addEventListener('click', function() {
    let answer = document.getElementById('answer_2ex');
    let answerInput = document.getElementById('answerInput_2ex');
    let correctAnswer = answerInput.getAttribute("data-correct-answer").toUpperCase();
    let expectedInput = answerInput.getAttribute("data-expected-input");
